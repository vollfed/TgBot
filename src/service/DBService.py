...
def save_user_context(user_id: int, transcript=None, title=None, language=None):
    current = get_user_context(user_id)

    # Use existing values if arguments are None
    ...
    with sqlite3.connect(DB_PATH) as conn:
        c = conn.cursor()
        c.execute("""
            INSERT INTO user_context (user_id, transcript, title, language)
            VALUES (?, ?, ?, ?)
            ON CONFLICT(user_id) DO UPDATE SET
                transcript=excluded.transcript,
                title=excluded.title,
                language=excluded.language
        """, (user_id, transcript, title, language))
        conn.commit()
...
def get_user_context(user_id: int):
    with sqlite3.connect(DB_PATH) as conn:
        c = conn.cursor()
        c.execute("SELECT transcript, title, language FROM user_context WHERE user_id=?", (user_id,))
        row = c.fetchone()
    if row:
        return {
            "transcript": row[0],
